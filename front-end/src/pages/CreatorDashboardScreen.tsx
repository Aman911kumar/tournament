...
const CreatorDashboardScreen = () => {
  ...
  const [tournaments, setTournaments] = useState<CreatorTournament[]>([]);
  const [query, setQuery] = useState("");
  const [status, setStatus] = useState<(typeof statusFilters)[number]>("all");
  const [deletingId, setDeletingId] = useState<string | null>(null);

  useEffect(() => {
    setTournaments(getCreatorTournaments());
  }, []);

  const stats = useMemo(() => {
    const totalEarnings = tournaments.reduce((sum, tournament) => sum + tournament.earnings, 0);
    const totalParticipants = tournaments.reduce((sum, tournament) => sum + tournament.participants, 0);
    const totalViews = tournaments.reduce((sum, tournament) => sum + tournament.views, 0);
  ...
  const handleDelete = async (tournament: CreatorTournament) => {
    ...
    try {
      ...
      const updated = deleteCreatorTournament(tournament.id);
      ...
      <div className="mx-auto w-full max-w-2xl px-4 sm:px-5 mb-5">
        ...
        <div className="space-y-3">
          ...
          {recentTournaments.map((tournament, i) => (
            <GlassCard key={tournament.id} neon delay={i * 0.06}>
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="text-right shrink-0">
                  <p className="text-xs font-heading font-bold text-accent">{formatCurrency(tournament.earnings)}</p>
      ...
      <div className="mx-auto w-full max-w-2xl px-4 sm:px-5">
        ...
        <div className="space-y-3">
          ...
          {filteredTournaments.map((tournament, index) => (
            <GlassCard key={tournament.id} delay={index * 0.04} className="relative overflow-hidden">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="text-[10px] text-muted-foreground mt-0.5">
                    {tournament.game} - {formatCurrency(tournament.prizePool.total)} prize
                  </p>
